package com.hxlk.centre.handle;

import com.hxlk.centre.service.CommandOperationService;
...
public class MonitorServiceHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        System.out.println(CommandOperationService.hexStr2Str(res));
        byte[] reg = new byte[buf.readableBytes()];
        /**readBytes：将缓冲区字节数组复制到新建的 byte 数组中
         * 然后将字节数组转为字符串
         * */
        buf.readBytes(reg);
        String body = new String(reg, "gbk");
        System.out.println( "(Recv) current_thread: " + Thread.currentThread().getName() + ", body: " + body);
//        System.out.println(CommandOperationService.hexStr2Str(body));
        /**回复消息
         * copiedBuffer：创建一个新的缓冲区，内容为里面的参数
         * 通过 ChannelHandlerContext 的 write 方法将消息异步发送给客户端
         * */

        byte[] sendWifiByte = commandOperationService.cmdOperation(reg,res);
//
        ByteBuf respByteBuf = Unpooled.copiedBuffer(sendWifiByte);
